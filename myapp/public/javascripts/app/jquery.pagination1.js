...
(function($) {
	 ...
	 */
	$.PaginationCalculator1 = function(maxentries, opts) {
		this.maxentries = maxentries;
		this.opts = opts;
	}

	$.extend($.PaginationCalculator1.prototype, {
		/**
	...
	$.PaginationRenderers.defaultRenderer = function(maxentries, opts) {
		this.maxentries = maxentries;
		this.opts = opts;
		this.pc = new $.PaginationCalculator1(maxentries, opts);
	}
	...
	// Extend jQuery
	$.fn.pagination1 = function(maxentries, opts) {
			...
			//goto
			$(".page-btn1").unbind("click");
			$(".page-btn1").on("click", function() {
				var allPage = $(".pages1 .allPage").text();
				//console.log(allPage);
				var goPage = $(".pages1 .page-go input").val() - 1; //跳转页数
				if($(".pages1 .page-go input").val()==""||$(".pages1 .page-go input").val()==0){
					return false;
				}
//				if(goPage == -1) {
//					goPage = 0;
//				}
				if(goPage > -1 && goPage < allPage) {
					...
					$("#Paginations").pagination1(allPage, opts);
					$("#Paginations").pagination1(allPage, opts);
					selectPage(goPage);
...
//					$("#Paginations").pagination(allPage, opts);
			...
			function paginationClickHandler(evt) {
				if($(evt.target).data('page_id') == undefined){
					if($(evt.target).parent().hasClass("prev") == true){
						$("body, html").scrollTop(0);
						if($(".pages1 #Paginations").css("display") == "block"){
							var links,
			...
			function selectPage(new_current_page) {
				...
				$(".pages1 #Paginations .pagination .prev").hover(function() {
				...
				$(".pages1 #Paginations .pagination .next").hover(function() {
					$(this).css("border", "1px solid #2cb82c");
				...
				$(".pages1  .page-btn1").hover(function() {
					$(this).css("border", "1px solid #2cb82c");				
				}, function() {
					$(this).css("border", "1px solid #e6e6e6");
				});
			...
			var pc = new $.PaginationCalculator1(maxentries, opts);
			var np = pc.numPages();
