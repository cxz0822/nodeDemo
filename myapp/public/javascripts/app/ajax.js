 ...
 */
function sturyZ(){
	var url = "http://onlineweb-2.boxuegu.com/";
...
sturyZ();
var bath = "http://onlineweb-2.boxuegu.com/";
...
var shareZo =
	...
	'<div class="zoneTitle clearfix">' +
	'<span>分享动态</span>' +
	'<i class="iconfont icon-guanbi"></i>' +
	'</div>' +
	'<div class="cont">' +
	'	<p class="cont1">' +
	'		<span data-topicId={{data.topic.id}}>{{data.topic.name}}</span><em>我有话说</em>' +
	'	</p>' +
	'	<div class="cont2">' +
	'		<textarea maxlength="200">{{data.quotation}}</textarea>' +
	'		<div class="jishu">' +
	'			<span class="js0">{{data.quotation.length}}</span><span>/200</span>' +
	'		</div>' +
	'	</div>' +
	'	<div class="shareImg">' +
	'			{{if data.type==3}}' +
	'			<div style="font-size:16px;color:#333;margin-bottom:14px">刚刚收到来自老师的批阅反馈</div>' +
	'			{{/if}}' +
	'		<div class="share0">' +
	'			{{if data.type==1}}' +
	'			{{#data.dec}}' +
	...
	'			<div class="share1Left">' +
	'				<img src="{{data.courseImg}}" />' +
	'			</div>' +
	'			<div class="share1Right">' +
	'				<p class="r1">{{data.courseName}}</p>' +
	'				<div class="r2">{{data.resName}}</div>' +
	'			</div>' +
	'		</div>' +
	'	</div>' +
	'	<div class="cz clearfix">' +
	'		<span class="fabu" id="b24" data-topicId={{data.topic.id}}>' +
	'		发布动态' +
	'	</span>' +
	'	<span class="quxiao">' +
	'		取消' +
...
function shareZone(type, resID, $this,sb) {
	var userid = "";
	$.ajax({
		type: "get",
	...
	$.ajax({
		type: "get",
		url: bath+"/sz/udc/template/" + type + "/res/" + resID + "",
		data: {
			token: userid
		},
		async: false,
		...
		success: function(data) {
			if(data.success == true) {
				if(data.resultObject.isShare != 1) {
					$(".shareZone").remove();
					$("body").append(template.compile(shareZo)({
						data: data.resultObject
					}));
					$(".shareZone").show();
					$(".zoneTitle i,.shareZone .quxiao").off("click").on("click", function() {
						$(".shareZone").hide();
					});
					$(".shareZone .cont2 textarea").off().on("input", function() {
						$(".shareZone .cont2 .js0").html($(this).val().length)
					});
					//点击分享
					$(".shareZone .fabu").off("click").on("click", function() {
						var topicID = $(this).attr("data-topicid");
						if($.trim($(".shareZone .cont2 textarea").val()) != "") {
							var con = $.trim($(".shareZone .cont2 textarea").val());
							$.ajax({
								type: "post",
								url: bath+"/sz/udc/type/" + type + "/res/" + resID + "/topic/" + topicID + "",
								async: true,
								data: {
									content: con,
									token: userid
								...
								success: function(a) {
									if(a.success == true) {
										if(a.resultObject == "保存成功") {
											rTips("发布成功");
											$(".shareZone").hide();
											$(".shareZ").hide();
											if($(".freeTable-list .hoverBorder").next().length == 0 && $(".freeTable-list .hoverBorder").parents(".freeTable-list").next().length == 0) {
					...
					if(type != 1) {
						$(".shareZ").off("click").on("click", function() {
							$(".shareZone").show();
						});
						$(".shareZ").css("display", "block");
					};
					if(type == 2) {
						$(".shareZ").css("display", "inline-block");
					};
				} else {
					$(".shareZ").hide();
					if(type==1){
						if(sb==1){
							...
							if($this.nextAll(".tj").length == 0) {
								if($this.parents(".jie").nextAll(".jie").length != 0) {
									var $m = $this.parents(".jie").nextAll(".jie");
									$m.find(".tj").eq(0).click();
								}else if($this.parents(".freeTable-list").nextAll(".freeTable-list").length != 0) {
									var $m = $this.parents(".freeTable-list").nextAll(".freeTable-list");
